...
function popUpRandomMole() {
  ...
  const moleHeads = document.querySelectorAll('.m-head');
  ...
  moleHead.classList.remove('m-head--hidden', 'm-head--whacked');
...
function hideMole(mole) {
  clickable = false;
  mole.classList.add('m-head--hidden');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  for (let moleHead of moleHeads) {
    moleHead.addEventListener('click', event => {
      ...
      // UNCOMMENT THIS LINE OF CODE WHEN DIRECTED
      event.target.classList.add('m-head--hidden');

      // UNCOMMENT THIS LINE OF CODE WHEN DIRECTED FOR THE BONUS
      event.target.classList.add('m-head--whacked');
    });
  }
});
